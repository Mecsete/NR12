...
(async ()=>{
  ...
  /* 24/09/2026: a conferencia de citacao e da IA (nos arquivos da pasta dela), nao do app */
  {
    console.log("\n[t176] citacao: o app nao confere; a instrucao manda a IA confirmar nos TXT/PDF");
    t("o app nao tem mais conferencia de citacao nem aviso de citacao nao conferida", ()=>{
      ["CITACOES_GUIAS","baseIAConferirCitacaoDaSolucao","citacaoNaoConferida","citacoesNaoConferidas","baseIACitacoesPermitidas","NÃO conferida (fora da tabela"]
        .forEach(x=> ok(HTML.indexOf(x) < 0, "ainda existe: " + x));
    });
    t("a instrucao manda confirmar cada citacao nos guias, TXT e PDF da pasta e tirar a que nao confirmar", ()=>{
      const iC = HTML.indexOf('"CITAÇÃO DE NORMA NA SOLUÇÃO — CINCO FONTES, NESTA ORDEM.');
      const cab = HTML.slice(iC, HTML.indexOf("\n", iC));
      ["CONFIRME cada citação nesses arquivos antes de deixá-la","_gerador_guia/normas_txt","trata EXATAMENTE da medida proposta",
       "O aplicativo não confere citação nenhuma na importação: a conferência é sua","Item que você não consegue confirmar nos arquivos sai da célula"]
        .forEach(x=> ok(cab.indexOf(x) > 0, "faltou: " + x));
    });
  }
